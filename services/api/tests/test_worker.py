...
def test_worker_completes_queued_job(tmp_path: Path) -> None:
    ...
    assert (
        tmp_path / "data" / "jobs" / response.job_id / "artifacts" / "completion" / "baseline_nearest_001.png"
    ...
    assert "completion/baseline_nearest_001.png" in explorer_bundle["viewer_assets"]
