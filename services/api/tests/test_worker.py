...
def test_worker_completes_queued_job(tmp_path: Path) -> None:
    ...
    assert (tmp_path / "data" / "jobs" / response.job_id / "artifacts" / "completion" / "baseline_nearest_001.svg").is_file()
    ...
    assert "completion/baseline_nearest_001.svg" in explorer_bundle["viewer_assets"]
